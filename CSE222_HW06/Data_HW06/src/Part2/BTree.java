...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
     ...
     * @param order - the size of a node
     * @throws IllegalArgumentException if order small then 3
     */
...
// Insert solution to programming exercise 1, section 5, chapter 9 here
    ...
    /**
     * binarySearch for Btree
     * But is not normal binarySearch
     * @param item for search
     * @param data for search item in data
     * @param height search which height
     * @param size size of array
     * @return founded index
     */
    public int binarySearch(E item, E[] data, int height, int size) {

       for(int i=0; i < size; ++i){
           if(item.compareTo(data[i]) == -1 || item.compareTo(data[i]) == 0){
               return i;
           }
       }
